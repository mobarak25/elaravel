				...
				<tbody>
					...
					@foreach($categories as $category)
						<tr>
							...
							<td class="center">
								<span class="label {{($category->publication_status == 1)? 'label-success':'label-warning'}}">{{($category->publication_status == 1)? 'Published':'Unpublished'}}</span>
							</td>
							<td class="center">
								<a class="btn {{($category->publication_status == 1)? 'btn-danger':'btn-success'}}" href="{{url('/active_unactive_cat/'.$category->category_id)}}">
									<i class="halflings-icon white {{($category->publication_status == 1)? 'thumbs-down':'thumbs-up'}}"></i>
								</a>
